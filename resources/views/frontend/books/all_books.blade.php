...
@section('meta')
<meta name="keywords" content="كتب,ابحاث,الشيخ,حمد,الهاجرى,دينية,اسلامية,تحميل">
<meta name="description" content="كتب وابحاث الشيخ الدكتور حمد بن محمد الهاجرى فى علوم الشريعة والفقة المقارن والسياسة الشرعية">
<meta name="author" content="{{ setting()->site_name }}">
@endsection
@section('title')
    {{ trans('frontend.books') }}
@endsection
@section('content')
    <!-- start row  with categories-->
    <?php
    $categories = \App\Models\BookCategory::tree();

    ?>
    <div class="headCategoriesImg">
        @if (setting()->book_banner)
    <img src="{{ asset('Files/settings/'.setting()->book_banner) }}" alt="{{ trans('frontend.books') }}" title="    {{ trans('frontend.books') }}
    " />
    {{-- @else
    <img src="{{ asset('frontend/img/book.png') }}" alt=""> --}}
@endif
    </div>
    ...
    <div class="row m-0 cardCat align-items-start">
        <div class="col-md-4 m-auto sideCategories">
            ...
            <div class="categoriesList">
                <div class="allLinks">
                    ...
                    <h2><a href="{{ route('frontend.books.all-books') }}" title=" {{ trans('frontend.all') }}">
                         {{ trans('frontend.all') }} <span style="color: var(--scoundry-color)">(<span style="color: var(--black-color)">{{\App\Models\Book::Active()->Publish()->count()}}</span>)</span></a></h2>
                </div>
                ...
                <div class="categoriesListUl">
                    @forelse ($categories as $category)
                        <ul>
                            ...
                            <a href="{{ route('frontend.books.category.books', $category->slug) }}" title="{{ $category->name }}">
                                {{ $category->name }} <span style="color: var(--scoundry-color)">(<span style="color: var(--black-color)">{{$category->books->where('status',1)->where('publish_date','<=',\Carbon\Carbon::now())->count()}}</span>)</span>
                            </a>


                            @include('frontend.books.subCategoryList', [
        ...
        <div class="col-md-8 my_card row allCategoriesCard">
            @forelse ($books as $book)
                <div class="col-sm-6 col-md-6 col-lg-4 categoriesCard">
                    <div class="card h-100">
                        ...
                        @else
                            <img src="{{ asset('frontend/img/books.webp') }}" alt="{{ $book->name }}"
                                title="{{ $book->name }}">
                        @endif
                        <div class="card-body">
                        ...
                        $check = App\Models\Wish::with('client')->where('client_id',$client_id)
                                ...
                                ->where('wishable_id',$book->id)->first();
                        ?>
                        @if ($check)
                            <button class="addToWish like" data-id="{{ $book->id }}">
                                <i class="fas fa-heart"></i>
                            </button>
                        @else
                            <button class="addToWish" data-id="{{ $book->id }}">
            ...
            @empty
                ...
                <h2 class="font-weight-bold mt-5 text-center">{{ trans('frontend.no-books') }}</h2>
            @endforelse
...
<script>
     $(document).ready(function() {
            ...
            $('#search').on('keyup',function(){
                ...
                // console.log(value);
                $.ajax({
                       url: " {{ route('frontend.books.book.search') }}" ,
                        type: "GET",
                        datType: "json",
                         data:{'search':value},
                    success:function(data)
