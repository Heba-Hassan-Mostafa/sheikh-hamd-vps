...
@section('content')
    ...
    <?php
    $categories = \App\Models\SpeechCategory::tree();

    ?>
    <div class="headCategoriesImg">
        @if (setting()->speech_banner)
            <img src="{{ asset('Files/settings/' . setting()->speech_banner) }}" alt="{{ trans('frontend.speeches') }}"
                title="{{ trans('frontend.speeches') }}" />
            {{-- @else
            <img src="{{ asset('frontend/img/speech.png') }}" alt=""> --}}
        @endif
    </div>
    ...
    <div class="row m-0 cardCat align-items-start">
        <div class="col-md-4 m-auto sideCategories">
            ...
            <div class="categoriesList">
                <div class="allLinks">
                    ...
                    <h2>
                        <a href="{{ route('frontend.speeches.all-speeches') }}" title="{{ trans('frontend.all') }}">
                            {{ trans('frontend.all') }} <span style="color: var(--scoundry-color)">(<span style="color: var(--black-color)">{{ \App\Models\Speech::Active()->Publish()->count()}}</span>)</span></a>
                    </h2>
                </div>
                ...
                <div class="categoriesListUl">
                    @forelse ($categories as $category)
                        <ul>
                            ...
                            <a href="{{ route('frontend.speeches.category.speeches', $category->slug) }}"
                                title=" {{ $category->name }}">
                                {{ $category->name }}<span style="color: var(--scoundry-color)">(<span style="color: var(--black-color)">{{ $category->speeches->where('status',1)->where('publish_date','<=',\Carbon\Carbon::now())->count() }}</span>)</span>
                            </a>


                            @include('frontend.speeches.subCategoryList', [
        ...
        <div class="col-md-8 my_card row allCategoriesCard">
            @forelse ($speeches as $speech)
                <div class="col-sm-6 col-md-6 col-lg-4 categoriesCard">
                    <div class="card h-100">
                        ...
                        @else
                            <img src="{{ asset('frontend/img/spee.webp') }}" alt="{{ $speech->name }}"
                                title="{{ $speech->name }}">
                        @endif
                        <div class="card-body">
                        ...
                        ?>
                        @if ($check)
                            <button class="addToWish like" data-id="{{ $speech->id }}">
                                <i class="fas fa-heart"></i>
                            </button>
                        @else
                            <button class="addToWish" data-id="{{ $speech->id }}">
            ...
            @empty
                ...
                <h2 class="font-weight-bold mt-5 text-center">{{ trans('frontend.no-speeches') }}</h2>
            @endforelse
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#search').on('keyup', function() {
                ...
                // console.log(value);
                $.ajax({
                    url: " {{ route('frontend.speeches.speech.search') }}",
                    type: "GET",
                    datType: "json",
                    data: {
                        'search': value
