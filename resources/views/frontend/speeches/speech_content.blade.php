...
@section('meta')
    ...
    <meta property="og:title" content="{{ $speech->name }}">
    <meta property="og:image" content="{{ $speech->image->file_name === null ? asset('frontend/img/spee.png') : asset('Files/image/Speeches/'.$speech->name.'/'.$speech->image->file_name) }}">
    <meta property="og:url" content="https://hamadalhajri.net/speeches/{{ $speech->slug }}">
    ...
    <div class="content">
        ...
        <div class="allCont row m-0">
            <div class="col-lg-9 contentTopic">
                ...
                <div class="contentDetails row m-0">
                    <div class="col-md-6">
                        <div class="filter mb-3">
                            ...
                            <span>
                                <a style="color: var(--main-color);font-weight:bold;margin-left: 8px;"
                                    ...
                                    title="{{ trans('frontend.speeches') }}">{{ trans('frontend.speeches') }}</a>
                            </span>
                            <i class="far fa-window-minimize dash-icon"></i>
                            <span>
                                <a style="color: var(--main-color);font-weight:bold;"
                                    href="{{ route('frontend.speeches.category.speeches', $speech->category->slug) }}"
                                    title="{{ $speech->category->name }}">{{ $speech->category->name }}</a>
                            </span>
                        </div>
                        ...
                        <div class="downCount d-flex">
                            ...
                            <p>
                                ...
                                <span class="downloaders">{{ $speech->download_count }}</span>
                            </p>
                        </div>
                        ...
                        <div class="col-11 socialShare">
                            ...
                            <button class="button"
                                data-sharer="twitter"
                                data-url="{{ route('frontend.speeches.speech_content', $speech->slug) }}">
                        ...
                        <div class="row m-0">
                                    ...
                                    @forelse ($speech->attachments as $file)
                                        <li>
                                            ...
                                            <a id="download-numbers" download title="{{ $file->file_name }}"
                                                href="{{ asset('Files/Pdf-Files/Speeches/' . $speech->name . '/' . $file->file_name) }}">
                                                {{ $file->file_name }}</a>
                                        </li>
                                    @empty
                            ...
                            $check = App\Models\Wish::with('client')
                                ...
                                ->where('wishable_id', $speech->id)
                                ->first();
                            ?>
                            <div class="col-md-12 col-lg-6 p-0 mb-2 loveContent d-flex">
                                @if ($check)
                                    <button class="addToWish like loveContent" data-id="{{ $speech->id }}">
                                        <i class="fas fa-heart"></i>
                                    </button>
                                @else
                                    <button class="addToWish loveContent" data-id="{{ $speech->id }}">
                    ...
                    <div class="col-md-6 contentImg">
                        ...
                        <div class="img-title">
                            <a href="{{ asset('Files/image/Speeches/' . $speech->name . '/' . $speech->image->file_name) }}">
                                <img src="{{ asset('Files/image/Speeches/' . $speech->name . '/' . $speech->image->file_name) }}"
                                class="img-fluid" title="{{ $speech->name }}" alt="{{ $speech->name }}" /> 
                            </a>
                        </div>
                           
                        @else
                        <div class="img-title">
                            <a href="{{ asset('frontend/img/spee.webp') }}">
                                <img src="{{ asset('frontend/img/spee.webp') }}" class="img-fluid" title="{{ $speech->name }}"
                                alt="{{ $speech->name }}" />
                            </a>
                        </div>
                ...
                <div class="comments">
                    ...
                    <div class="allCommentContent">
                        @forelse ($speech->comments as $comment)
                            @if ($comment->status == 1)
            ...
            <div class="col-lg-3 sideDefultLinks">
                <h2> {{ trans('frontend.random-speech') }}</h2>
                <div class="row m-0">
                    @forelse ($randomSpeeches->except($speech->id) as $random)
                    @if ($random->speech_category_id == $speech->speech_category_id)
                        <div class="col-12 categoriesCard">
                            <div class="card h-100">
                                @if (!empty($random->image->file_name))
                                    <img src="{{ asset('Files/image/Speeches/' . $random->name . '/' . $random->image->file_name) }}"
                                        class="card-img-top" title="{{ $random->name }}" alt="{{ $random->name }}" />
                                @else
                                    <img src="{{ asset('frontend/img/spee.webp') }}" class="card-img-top"
                                        title="{{ $random->name }}" alt="{{ $speech->name }}" />
                                @endif
                                <div class="card-body">
                                    ...
                                    <div class="text-center m-2">
                                        <a href="{{ route('frontend.speeches.speech_content', $random->slug) }}"
                                            class="btn-card-more"
                    ...
                    @empty
                        ...
                        <span>{{ trans('frontend.no-random-speeches') }}</span>
                    @endforelse
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('.addToWish').on('click', function() {
                ...
                if (id) {
                    $.ajax({
                        url: " {{ url('/speeches/add/wishlist/') }}/" + id,
                        type: "GET",
                        datType: "json",
    ...
    <script>
        $(document).ready(function() {
            $('#save_comment').on('click', function(e) {
                ...
                $.ajax({
                    url: "{{ route('frontend.speeches.add.comment', $speech->id) }}",
                    type: "POST",
                    datType: "json",
                    data: {
                        '_token': "{{ csrf_token() }}",
                        ...
                        'commentable_id': '{{ $speech->id }}',
                        'commentable_type': 'App\Models\speech',
