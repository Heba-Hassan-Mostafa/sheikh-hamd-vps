...
@section('meta')
    ...
    <meta property="og:image" content="{{ $article->image->file_name === null ? asset('frontend/img/art.png') : asset('Files/image/Articles/'.$article->name.'/'.$article->image->file_name) }}">
    <meta property="og:url" content="https://hamadalhajri.net/articles/{{ $article->slug }}">
    ...
    <div class="content">
        ...
        <div class="allCont row m-0">
            <div class="col-lg-9 contentTopic">
                ...
                <div class="contentDetails row m-0">
                    <div class="col-md-6">
                        <div class="filter mb-3">
                            ...
                            <span>
                                <a style="color: var(--main-color);font-weight:bold;margin-left: 8px;"
                                    ...
                                    title="{{ trans('frontend.articles') }}">{{ trans('frontend.articles') }}</a>
                            </span>
                            <i class="far fa-window-minimize dash-icon"></i>
                            <span>
                                <a style="color: var(--main-color);font-weight:bold;"
                                    href="{{ route('frontend.articles.category.articles', $article->category->slug) }}"
                        ...
                        <div class="downCount d-flex">
                            ...
                            <p>
                                ...
                                <span class="downloaders">{{ $article->download_count }}</span>
                            </p>
                        </div>
                        ...
                        <div class="col-11 socialShare">
                            ...
                            <button class="button"
                                data-sharer="twitter"
                                data-url="{{ route('frontend.articles.article_content', $article->slug) }}">
                        ...
                        <div class="row m-0">
                                    ...
                                    @forelse ($article->attachments as $file)
                                        <li>
                                            ...
                                            <a id="download-numbers" download title="{{ $file->file_name }}"
                                                href="{{ asset('Files/Pdf-Files/Articles/' . $article->name . '/' . $file->file_name) }}">
                                                {{ $file->file_name }}</a>
                                        </li>
                                    @empty
                            ...
                            $check = App\Models\Wish::with('client')
                                ...
                                ->where('wishable_id', $article->id)
                                ->first();
                            ?>
                            <div class="col-md-12 col-lg-6 p-0 mb-2 loveContent d-flex">
                                @if ($check)
                                    <button class="addToWish like loveContent" data-id="{{ $article->id }}">
                                        <i class="fas fa-heart"></i>
                                    </button>
                                @else
                                    <button class="addToWish loveContent" data-id="{{ $article->id }}">
                    ...
                    <div class="col-md-6 contentImg">
                        ...
                        <div class="img-title">
                            <a href="{{ asset('Files/image/Articles/' . $article->name . '/' . $article->image->file_name) }}">
                               <img src="{{ asset('Files/image/Articles/' . $article->name . '/' . $article->image->file_name) }}"
                                class="img-fluid" title="{{ $article->name }}" alt="{{ $article->name }}" /> 
                            </a>
                        </div>
                            
                        @else
                        <div class="img-title">
                            <a href="{{ asset('frontend/img/art.webp') }}">
                              <img src="{{ asset('frontend/img/art.webp') }}" class="img-fluid" title="{{ $article->name }}"
                                alt="{{ $article->name }}" />  
                            </a>
                        </div>
            ...
            <div class="col-lg-3 sideDefultLinks">
                <h2> {{ trans('frontend.random-article') }}</h2>
                <div class="row m-0">
                    @forelse ($randomArticles->except($article->id) as $random)
                        @if ($random->article_category_id == $article->article_category_id)
                            <div class="col-12 categoriesCard">
                                <div class="card h-100">
                                    @if (!empty($random->image->file_name))
                                        <img src="{{ asset('Files/image/Articles/' . $random->name . '/' . $random->image->file_name) }}"
                                            class="card-img-top" title="{{ $random->name }}"
                                            alt="{{ $random->name }}" />
                                    @else
                                        <img src="{{ asset('frontend/img/art.webp') }}" class="card-img-top"
                                            title="{{ $random->name }}" alt="{{ $article->name }}" />
                                    @endif
                                    <div class="card-body">
                                        ...
                                        <div class="text-center m-2">
                                            <a href="{{ route('frontend.articles.article_content', $random->slug) }}"
                                                class="btn-card-more"
                        ...
                        @empty
                            ...
                            <span>{{ trans('frontend.no-random-articles') }}</span>
                        @endforelse
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('.addToWish').on('click', function() {
                ...
                if (id) {
                    $.ajax({
                        url: " {{ url('/articles/add/wishlist/') }}/" + id,
                        type: "GET",
                        datType: "json",
    ...
    <script>
        $(document).ready(function() {
            $('#save_comment').on('click', function(e) {
                ...
                $.ajax({
                    url: "{{ route('frontend.articles.add.comment', $article->id) }}",
                    type: "POST",
                    datType: "json",
                    data: {
                        '_token': "{{ csrf_token() }}",
