<div class="table-responsive" wire:ignore>
    <table class="table text-md-nowrap" id="example1">
        ...
        <tbody wire:sortable="updateCategoryOrder">
            ...
            <tr class="reOrder" wire:sortable.item="{{ $category->id }}" wire:key="speechCategory-{{ $category->id }}"
                wire:sortable.handle>
                <td>{{ $category->id }}</td>
                <td>{{ $category->name }}</td>
                <td>{{ $category->speeches->count() }}</td>
                <td>{{ $category->status() }}</td>
                <td>{{ $category->created_at->format('Y-m-d') }}</td>
                <td>
                    <div class="btn-group">

                        <a href="{{ route('admin.speech-categories.edit', $category->id) }}"
                            class="btn btn-info btn-sm" role="button" aria-pressed="true">
                            <i class="fa fa-edit"></i></a>


                    <form action="{{ route('admin.speech-categories.destroy', $category->id) }}" method="POST"
                        class="dnone">
                        @csrf
                        @method('Delete')
                    ...
                    {{-- <a href="{{ route('admin.speech-categories.show', $category->id) }}"
